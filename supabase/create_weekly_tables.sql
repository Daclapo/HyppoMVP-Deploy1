-- Verificar y crear tablas para el apartado "Semanal" si no existen

-- Verificar si la tabla weekly_posts existe y crearla si no
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'weekly_posts'
  ) THEN
...
-- Verificar si la tabla weekly_reflections existe y crearla si no
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'weekly_reflections'
  ) THEN
...
-- Verificar si la tabla weekly_reflection_comments existe y crearla si no
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'weekly_reflection_comments'
  ) THEN
